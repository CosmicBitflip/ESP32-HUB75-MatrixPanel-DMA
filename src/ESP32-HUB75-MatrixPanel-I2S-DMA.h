...
/***************************************************************************************/
// CIE 1931 lightness correction lookup tables - auto-generated by tools/generate_cie_luts.py
// Contains native bit-depth LUTs (4, 6, 7, 8, 10, 12-bit) for optimal performance and quality.
// BCM bit-extraction uses a plain (1 << colour_depth_idx) mask, which is correct because
// DO_BRIGHTNESS_COMPENSATION() always produces red_val/green_val/blue_val in the native
// bit-depth range [0 .. (2^depth - 1)].
#include "cie_luts.h"
...
private:
  // these were previously handled as defines (PIXEL_COLOR_DEPTH_BITS)
  // to make them changeable after compilation, they are now part of the config
