...
/***************************************************************************************/
// CIE 1931 lightness correction lookup table (16-bit output, indexed by 8-bit input).
// The BCM bit-extraction in updateMatrixDMABuffer / hlineDMA / vlineDMA tests bits
// [MASK_OFFSET .. MASK_OFFSET+depth-1] of these 16-bit values via PIXEL_COLOR_MASK_BIT,
// so the table must remain 16-bit (0-65535 range).  Do not replace with a native-depth
// table without also updating the mask macro and all four pixel-write call sites.
#ifndef NO_CIE1931
...
// Mask macro: selects bit 'color_depth_index' of a 16-bit lumConvTab value.
// MASK_OFFSET = 16 - getPixelColorDepthBits() aligns the active BCM bits to the
// top 'depth' bits of the 16-bit table output so that every bit depth (6, 8, …)
// works correctly without changing the table itself.
